...
static void flow_color_led() {
	// color: 2bit each
	static uint8_t current[3] = {16,16,6}; // 
	static uint8_t last[3] = {16,16,16};
	const unsigned max_count = 25;
	static unsigned count = max_count, frame_count = 15;
	const uint16_t light_bitmap[17] = {
    0x0000,  // 0/16 bits set: 0000000000000000
    0x8000,  // 1/16 bits set: 1000000000000000
    0x8080,  // 2/16 bits set: 1000000010000000
    0x8888,  // 3/16 bits set: 1000100010001000  
    0xA8A8,  // 4/16 bits set: 1010100010101000
    0xAAAA,  // 5/16 bits set: 1010101010101010
    0xAAAE,  // 6/16 bits set: 1010101010101110
    0xEEEE,  // 7/16 bits set: 1110111011101110
    0xEEEF,  // 8/16 bits set: 1110111011101111
    0xEFEF,  // 9/16 bits set: 1110111111101111
    0xFFEF,  // 10/16 bits set: 1111111111101111
    0xFFF7,  // 11/16 bits set: 1111111111110111
    0xFFFB,  // 12/16 bits set: 1111111111111011
    0xFFFD,  // 13/16 bits set: 1111111111111101
    0xFFFE,  // 14/16 bits set: 1111111111111110
    0xFFFF,  // 15/16 bits set: 1111111111111111
    0xFFFF   // 16/16 bits set: 1111111111111111
	};
	uint16_t r,g,b; unsigned tmp;
	r = ((uint16_t)current[0]*count + (uint16_t)last[0]*(max_count-count)) / max_count;
	g = ((uint16_t)current[1]*count + (uint16_t)last[1]*(max_count-count)) / max_count;
	b = ((uint16_t)current[2]*count + (uint16_t)last[2]*(max_count-count)) / max_count;
	tmp = 1<<frame_count;
	if(light_bitmap[r] & tmp) {LED1_ON} else {LED1_OFF}
	if(light_bitmap[g] & tmp) {LED2_ON} else {LED2_OFF}
	if(light_bitmap[b] & tmp) {LED3_ON} else {LED3_OFF}
	// forward
	if (++frame_count >= 16) {
		frame_count = 0;
		if(++count >= max_count) { // update color
			count = 0;
			last[0] = current[0];
			last[1] = current[1];
			last[2] = current[2];
			tmp = rand();
			current[0] = tmp%17; tmp/=17;
			current[1] = tmp%17; tmp/=17;
			current[2] = tmp%17;
		}
	}
